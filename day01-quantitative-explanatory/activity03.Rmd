...
```{r, load-data}
#hfi<-readr::read_tsv('https://www.openintro.org/data/csv/hfi.csv')
hfi<-read.csv('~/Desktop/CIS635W23/STA 631 W24/activity3/hfi.csv')
hfi_2016<-hfi%>%
  filter(`year` == 2016) 
```
...
hfi_2016 %>% 
  select(pf_score, pf_expression_control, pf_rol_civil) %>% 
  ggpairs()
```
...
mlr_mod <- lm_spec %>% 
fit(pf_score ~ pf_expression_control + pf_rol_civil, data = hfi_2016)
...
fig <- plot_ly(hfi_2016, x = ~pf_score, y = ~pf_expression_control, z = ~pf_rol_civil, color = ~pf_score, colors = c('#4AC6B7', '#C61951'))
fig <- fig %>% add_markers()
fig <- fig %>% layout(scene = list(xaxis = list(title = 'Personal Freedom Score'),
                     yaxis = list(title = 'Expression Cotrol'),
                     zaxis = list(title = 'Civil Control')))
